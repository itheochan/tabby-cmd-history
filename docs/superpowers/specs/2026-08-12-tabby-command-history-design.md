...
出现无法识别的控制序列，或执行结果无法由插件观察的操作（包括 Shell 原生 Tab 补全）后，缓冲区立即清空并标记为不可信，隐藏预测且不保留可能已过期的文本。不可信缓冲区不得持久化。按 Enter 时，插件用重写前的可信文本作为锚点，从当前可见逻辑行恢复完整命令（补全会扩展锚点，命令即锚点最后一次出现位置到行尾的后缀）；恢复结果仍必须通过第 7.3 节的严格可见回显检查才会记录，无法恢复时不记录。Enter 或 `Ctrl+C` 将状态重置为空且可信的新缓冲区。
...
- Tab 始终转发给 Shell；插件清空重建缓冲区以丢弃可能已过期的文本，并在 Enter 时用锚点从可见行恢复完整命令。
- `Ctrl+C` 始终透传并清空插件状态。
