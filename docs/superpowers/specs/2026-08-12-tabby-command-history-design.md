...
出现无法识别的控制序列，或执行结果无法由插件观察的操作（包括 Shell 原生 Tab 补全）后，缓冲区立即清空并标记为不可信，隐藏预测且不保留可能已过期的文本。不可信缓冲区不得持久化。Shell 补全的输出到达（frontend content update）后，插件用重写前的可信文本作为锚点，从当前可见逻辑行重建缓冲区（补全会扩展锚点，命令即锚点最后一次出现位置到行尾的后缀），重建成功后预测恢复；无输出时由兜底定时器触发同样的重建。若重建失败（无法观察到可信行），缓冲区保持不可信，按 Enter 时插件仍用锚点恢复完整命令，恢复结果必须通过第 7.3 节的严格可见回显检查才会记录。Enter 或 `Ctrl+C` 将状态重置为空且可信的新缓冲区。
...
终端处于 alternate-screen 模式期间，捕获和预测均禁用，运行时状态被清空，所有输入字节直接透传。返回 normal screen 后，从空且可信的缓冲区重新开始。进入或退出 alternate screen 不会取消 Enter 后尚未开始持久化的待处理命令记录：分页器等应用会立即进入 alternate screen，但命令本身已在该 connection 提交。
...
- Tab 始终转发给 Shell；插件清空重建缓冲区以丢弃可能已过期的文本，在 Shell 补全输出到达后从可见行重建缓冲区并恢复预测，Enter 时用锚点兜底恢复完整命令。
- `Ctrl+C` 始终透传并清空插件状态。
